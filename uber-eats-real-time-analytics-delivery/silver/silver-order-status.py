...
WATERMARK_DURATION = "10 minutes" # HOW LONG TO WAIT FOR LATE DATA
DELAY_THRESHOLD_MINUTES = 15 # CLASSIFY ORDERS TAKING MORE THAN 15 MINUTES AS DELAYED
...
@dlt.table(
    ...
    comment = "Order enriched with delivery status and performance metrics",
    table_properties = {
        "quality": "silver",
        "layer" : "integration"
    }
...
def silver_order_status():
    ...
    return(
        ...
        (status_df.dt_current_timestamp <= orders_df.dt_current_timestamp + F.expr("INTERVAL 2 HOURS")), #Status updates must arrive within 2 hours of order creation
        "left"
        )
        .select(
                orders_df.order_id,
